...
ALUMNOS: 
		...
		ROSALES VELAZQUEZ VICTOR MANUEL
------------------------------------------------------------------------------
ASIGNATURA:
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "TBBT_Xmas", nullptr, nullptr);
	...
	Model Foco1((char*)"Models/Foco/Foco3.obj");
...
//		glEnable(GL_BLEND);
	//	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); //NECESARIO PARA EFECTO TRASLUCIDO
		...
		model = glm::translate(model, glm::vec3(15.0f, 0.0f, 0.0f)); /**********************FOCO VERDE********************/
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		Foco1.Draw(lightingShader);

		model = glm::translate(model, glm::vec3(15.0, 0.0f, 0.0f)); /**********************FOCO AMARILLO********************/
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		Foco2.Draw(lightingShader);
		
		model = glm::translate(model, glm::vec3(15.0, 0.0f, 0.0f)); /**********************FOCO ROJO********************/
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_SPACE])
	{
		...
		if (active)
		{
			Light1 = glm::vec3(0.0f, 0.0f, 5.0f);//Blue
			Light2 = glm::vec3(0.0f, 0.7f, 0.0f);//Green
			Light3 = glm::vec3(0.7f, 0.7f, 0.0f);//Yellow
			Light4 = glm::vec3(0.7f, 0.0f, 0.0f);//Red
		}
		else
		{
			Light1 = glm::vec3(0);
