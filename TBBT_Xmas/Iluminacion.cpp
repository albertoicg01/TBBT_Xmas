...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light.ambient"), 0.3f, 0.3f, 0.3f); //gris oscuro
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light.diffuse"), 1.0f, 1.0f, 1.0f);//blanco oscuro
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light.specular"), 0.0f, 0.0f, 1.0f); //azul

        // Set material properties establecer el material 
        glUniform3f(glGetUniformLocation(lightingShader.Program, "material.diffuse"), 1.0f, 0.0f, 0.0f);
        glUniform3f(glGetUniformLocation(lightingShader.Program, "material.specular"), 0.1f, 0.1f, 0.1f); // Specular doesn't have full effect on this object's material
        glUniform1f(glGetUniformLocation(lightingShader.Program, "material.shininess"), 90.0f);//El tamanio del puntito. Puede llegar de 1 a 100
                                                                                           //Entre mas bajo, mas brillo, mas se quema
        ...
        glm::mat4 model(1);
        // model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f)); Escala para cubos
        ...
        glUniform3f(glGetUniformLocation(lightingShader.Program, "material.specular"), 0.8f, 0.8f, 0.8f); // Specular doesn't have full effect on this object's material
        ...
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light.specular"), 0.0f, 1.0f, 0.0f);
